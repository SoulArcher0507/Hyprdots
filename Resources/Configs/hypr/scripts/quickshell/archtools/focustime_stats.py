...
def build_query(base_query, date_params, app_filter):
    if app_filter:
        return base_query + " AND app_class = ?", date_params + (app_filter,)
...
def main():
    ...
    if not os.path.exists(DB_PATH):
        print(json.dumps({
            ...
            "apps": [], "week_apps": [], "week": [], "month": [], "hourly": [0] * 48,
    ...
    q, p = build_query('SELECT SUM(seconds) FROM focus_log WHERE log_date = ?',
    ...
    for i in range(7):
        ...
        week_data.append({"date": d_str, "day": days_str[i], "total": week_map.get(d_str, 0),
                          "is_target": d_str == target_date_str})
    ...
    hourly_data = [0] * 48
    try:
        q, p = build_query('SELECT hour, SUM(seconds) FROM focus_hourly WHERE log_date = ?',
                            (target_date.isoformat(),), app_filter)
        ...
        q, p = build_query('SELECT interval_idx, SUM(seconds) FROM focus_intervals WHERE log_date = ?',
                            (target_date.isoformat(),), app_filter)
    ...
    result = {
        ...
        "week_range": week_range_str, "yesterday": yesterday_seconds,
