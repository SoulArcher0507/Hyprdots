...
STATE_FILE = os.path.join(CACHE_DIR, "focustime_state.json")
...
def get_icon_search_dirs(icon_name=""):
    search_dirs = []

    if icon_name.startswith("steam_icon_"):
        search_dirs.extend([
            ...
            os.path.expanduser("~/.local/share/icons/hicolor/22x22/apps"),
...
def resolve_icon_reference(icon_name):
    if not icon_name:
        return ""
    ...
    for directory in get_icon_search_dirs(icon_name):
        for extension in ("svg", "png", "xpm"):
            candidate = os.path.join(directory, f"{icon_name}.{extension}")
...
def get_app_icon(app_class):
    ...
    if app_class_lower in DESKTOP_CACHE_ICON:
        return resolve_icon_reference(DESKTOP_CACHE_ICON[app_class_lower])
    if base_class in DESKTOP_CACHE_ICON:
        return resolve_icon_reference(DESKTOP_CACHE_ICON[base_class])
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS focus_log
                 (log_date TEXT, app_class TEXT, seconds INTEGER, app_title TEXT,
                  PRIMARY KEY (log_date, app_class))''')
    c.execute('CREATE INDEX IF NOT EXISTS idx_log_date ON focus_log(log_date)')
    c.execute('''CREATE TABLE IF NOT EXISTS focus_hourly
                 (log_date TEXT, hour INTEGER, app_class TEXT, seconds INTEGER,
                  PRIMARY KEY (log_date, hour, app_class))''')
    c.execute('''CREATE TABLE IF NOT EXISTS focus_intervals
                 (log_date TEXT, interval_idx INTEGER, app_class TEXT, seconds INTEGER,
                  PRIMARY KEY (log_date, interval_idx, app_class))''')
    c.execute('''CREATE TABLE IF NOT EXISTS focus_minutes
                 (log_date TEXT, minute_idx INTEGER, app_class TEXT, seconds INTEGER,
...
def listen_hyprland_ipc():
    ...
    sock_path = os.path.join(XDG_RUNTIME, "hypr", hypr_sig, ".socket2.sock") if XDG_RUNTIME else ""
...
class DaemonTracker:
    ...
    def full_sync(self, target_date):
        ...
        for i in range(7):
            ...
            week_data.append({"date": d_str, "day": days_str[i], "total": week_map.get(d_str, 0),
                              "is_target": d_str == target_date.isoformat()})
        ...
        for i in range(1, num_days + 1):
            ...
            month_data.append({"date": d_str, "total": month_map.get(d_str, 0),
                               "is_target": d_str == target_date.isoformat()})

        hourly_data = [0] * 48
        try:
            c.execute('SELECT hour, SUM(seconds) FROM focus_hourly WHERE log_date = ? GROUP BY hour',
                      (target_date.isoformat(),))
            for hr, secs in c.fetchall():
                if 0 <= hr <= 23:
                    hourly_data[hr * 2] += secs
            c.execute('SELECT interval_idx, SUM(seconds) FROM focus_intervals WHERE log_date = ? GROUP BY interval_idx',
                      (target_date.isoformat(),))
        ...
        try:
            c.execute('''SELECT log_date, hour, SUM(seconds) FROM focus_hourly
        ...
        self.cached_json = {
            ...
            "hourly": hourly_data, "week_heatmap": week_heatmap
        }
